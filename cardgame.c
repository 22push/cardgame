...
{
	system("cls");
    //this function is inbuilt in the windows.h library and
    //it is used for changing the background colors.	
	system("Color 9F");
   ...
   //the 'goto' to a labeled statement in the same function.
	...
	// sleep () function in C allows the users to wait
	// for a current thread for a specific time and here the arguments represents seconds.
	sleep(1);
	...
	// getch () function in a C/ C++ program to hold the output screen for some time until 
	// the user passes a key from the keyboard to exit the console screen.
	char key = toupper(getch());

	if (key != 'Q' && key != 'P' && key != 'R') goto Intro;
	// The exit(1) function is a type of function with a return type without an argument and
	// it is usually used for abnormal execution.
	// It's defined by the stdlib header file.
	if (key == 'Q')	exit(1);
	if (key == 'R')
	{
	// system("cls") function is used to clear the output console screen.
		system("cls");
		rules();
		goto home;
	}
	...
	while (key == 'P'){
		...
		printf("\nDrawing cards...");
		sleep(2);
		// srand is a random number generator function which will 
		// randomize the number produced by rand function.
		// if you are using only rand function then u will get 
		// a random number but same at all times. 
		...
		if ((rand1 > rand2 && rand2 != 1 ) || (rand1 == 1)) // we want rand1 as a smallest card( not ACE).
		{
			int temp = rand1;
			rand1 = rand2;
			rand2 = temp;
...
//			flagAceDD = 0;  dealer can change the other - check later
		...
		if (N_14) //check weather sumN is 14 or not.
		{
		...
		if ( (rand1 == 3 && rand2 == 1) || (rand1 == 1 && rand2 == 3))
		{
			sumD = 14;
			D_14 = 1;
		}
